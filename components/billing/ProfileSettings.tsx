...
export const ProfileSettings: React.FC<ProfileSettingsProps> = ({ user, onUserUpdate, onSetGlobalSuccess }) => {
    ...
    return (
        <div className="space-y-12">
            ...
            <form onSubmit={handleProfileUpdate} className="bg-gray-800/50 rounded-lg p-6 border border-gray-700">
            ...
            {user.authProvider === 'email' && (
                <form onSubmit={handlePasswordChange} className="bg-gray-800/50 rounded-lg p-6 border border-gray-700">
...
const InfoItem: React.FC<{ label: string; value: string; isCapitalized?: boolean; }> = ({ label, value, isCapitalized }) => (
    <div className="bg-gray-900/50 p-3 rounded-md">
        ...
        <p className={`font-semibold text-gray-200 ${isCapitalized ? 'capitalize' : ''}`}>{value}</p>
    </div>
);
