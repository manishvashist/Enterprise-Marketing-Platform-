    ...
    return (
        <div className="bg-slate-800/50 rounded-xl shadow-lg p-6 md:p-8 border border-white/10">
            <div className="flex justify-between items-start">
                <div>
                    <h2 className="text-2xl font-bold mb-1 text-white">Generative Media Planner</h2>
                    <p className="text-gray-400">The AI has generated a plan based on your campaign. You can adjust the parameters below and regenerate.</p>
                </div>
                <button 
                    onClick={() => setIsExpanded(!isExpanded)}
                    className="flex-shrink-0 ml-4 px-4 py-2 bg-slate-700 text-sm font-semibold text-white rounded-md hover:bg-slate-600 transition-colors flex items-center"
                >
                ...
                <div className="mt-6 pt-6 border-t border-slate-700 animate-fade-in">
                    ...
                    <div className="mt-8 flex justify-end">
                        <button
                            ...
                            className="w-full sm:w-auto px-8 py-3 bg-indigo-600 text-white font-bold rounded-lg hover:bg-indigo-500 disabled:bg-slate-600 disabled:cursor-not-allowed transition-all shadow-lg shadow-indigo-600/20 hover:shadow-xl hover:shadow-indigo-600/30 flex items-center justify-center"
                        >
                            {isLoading ? (
                                <>
...
const TextAreaField: React.FC<React.TextareaHTMLAttributes<HTMLTextAreaElement> & { label: string }> = ({ label, ...props }) => (
    <div>
        ...
        <textarea 
            ...
            {...props} 
            rows={3}
            className="w-full p-3 bg-slate-900 border border-slate-700 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all resize-none text-gray-200 placeholder-gray-500"
        />
    </div>
);