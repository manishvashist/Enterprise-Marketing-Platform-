...
-- FIX: Replaced invalid JavaScript-style comment (//) with a valid SQL comment (--) inside the SQL string.
-- FIX: Swapped RLS policy conditions to \`auth.uid() = ...\` for linter compatibility and consistency.
...
DROP POLICY IF EXISTS "Users can view and delete their non-deleted campaigns." ON public.campaigns;
DROP POLICY IF EXISTS "Users can insert their own campaigns." ON public.campaigns;
DROP POLICY IF EXISTS "Users can update their own non-deleted campaigns." ON public.campaigns;
-- Drop new policies for idempotency
DROP POLICY IF EXISTS "Users can view their own non-deleted campaigns." ON public.campaigns;
...
-- FIX: Replaced invalid JavaScript-style comment (//) with a valid SQL comment (--) inside the SQL string.
-- FIX: Swapped RLS policy conditions to \`auth.uid() = ...\` for linter compatibility and consistency.
-- 1. SELECT: Users can only see their campaigns that are NOT soft-deleted.
CREATE POLICY "Users can view their own non-deleted campaigns." ON public.campaigns
FOR SELECT USING (auth.uid() = user_id AND is_deleted = false);

-- 2. INSERT: Users can create campaigns for themselves.
CREATE POLICY "Users can insert their own campaigns." ON public.campaigns
FOR INSERT WITH CHECK (auth.uid() = user_id);

-- 3. UPDATE: Users can update their own campaigns.
-- This policy uses a \`USING (auth.uid() = user_id)\` clause to specify that
-- users can only update rows they own. This is more specific than \`USING (true)\`
-- and is a robust way to avoid conflicts with the SELECT policy during soft deletes,
-- FIX: Replaced invalid JavaScript-style comment (//) with a valid SQL comment (--) inside the SQL string.
-- as it scopes the update operation to the user's own data from the start.
CREATE POLICY "Users can update their own campaigns." ON public.campaigns
FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- 4. DELETE: Users can hard-delete any of their own campaigns.
